...
MAJORMINOR="${VER%.*}"
SRC="$DIST/$MAJORMINOR"
# сценарий «без клона репо»: curl -fsSL install-mod.sh | bash — артефакты качаются
# единым архивом mod-<major>.tar.gz из GitHub Releases (переопределяется MOD_DIST_URL)
BASE_URL="${MOD_DIST_URL:-https://github.com/0x3654/PulseSync-mod/releases/latest/download}"
