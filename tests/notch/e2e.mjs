...
// Запуск: node tests/notch/e2e.mjs
...
// базовый CDP можно переопределить: NOTCH_CDP=http://127.0.0.1:9223 node tests/notch/e2e.mjs
...
// 0) меню и ссылки требуют активного трека — на тестовой машине плеер в simple
...
const barVariant = (await mw.j(`(() => (document.querySelector('button[data-test-id="PLAYERBAR_DESKTOP_CONTEXT_MENU_BUTTON"]') ? 'classic' : document.querySelector('button[data-test-id="VIBE_CONTEXT_MENU_BUTTON"]') ? 'vibe' : 'none'))()`));
...
for (let attempt = 0; attempt < 4 && !actOk; attempt += 1) {
    ...
    if (barVariant === 'classic') {
        ...
        const path = await mw.j(`location.pathname + location.search`);
        actOk = path.startsWith('/album') || path.startsWith('/track');
        if (attempt === 3) check('Go to track осуществляет переход', actOk, path);
    }
}
