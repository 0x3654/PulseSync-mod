'use strict';
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
    [2146],
    {
        449: (e, r, t) => {
            t.d(r, { A: () => f });
            var n = t(85599),
                u = t(23281);
            let f = function (e) {
                return (0, u.A)((0, n.A)(e).toLowerCase());
            };
        },
        6028: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = t(90780);
            let u = function (e) {
                return (0, n.A)(e) ? e : [];
            };
        },
        6762: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r) {
                return e.has(r);
            };
        },
        12134: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                return e != e;
            };
        },
        13209: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                return e;
            };
        },
        14752: (e, r, t) => {
            t.d(r, { A: () => L });
            var n = t(8994),
                u = t(18861),
                f = t(91961),
                o = t(6762);
            let a = function (e, r, t, n, a, i) {
                var c = 1 & t,
                    d = e.length,
                    l = r.length;
                if (d != l && !(c && l > d)) return !1;
                var A = i.get(e),
                    s = i.get(r);
                if (A && s) return A == r && s == e;
                var v = -1,
                    h = !0,
                    p = 2 & t ? new u.A() : void 0;
                for (i.set(e, r), i.set(r, e); ++v < d;) {
                    var g = e[v],
                        b = r[v];
                    if (n) var x = c ? n(b, g, v, r, e, i) : n(g, b, v, e, r, i);
                    if (void 0 !== x) {
                        if (x) continue;
                        h = !1;
                        break;
                    }
                    if (p) {
                        if (
                            !(0, f.A)(r, function (e, r) {
                                if (!(0, o.A)(p, r) && (g === e || a(g, e, t, n, i))) return p.push(r);
                            })
                        ) {
                            h = !1;
                            break;
                        }
                    } else if (!(g === b || a(g, b, t, n, i))) {
                        h = !1;
                        break;
                    }
                }
                return (i.delete(e), i.delete(r), h);
            };
            var i = t(32024),
                c = t(31357),
                d = t(48289),
                l = t(45124),
                A = t(28566),
                s = i.A ? i.A.prototype : void 0,
                v = s ? s.valueOf : void 0;
            let h = function (e, r, t, n, u, f, o) {
                switch (t) {
                    case '[object DataView]':
                        if (e.byteLength != r.byteLength || e.byteOffset != r.byteOffset) break;
                        ((e = e.buffer), (r = r.buffer));
                    case '[object ArrayBuffer]':
                        if (e.byteLength != r.byteLength || !f(new c.A(e), new c.A(r))) break;
                        return !0;
                    case '[object Boolean]':
                    case '[object Date]':
                    case '[object Number]':
                        return (0, d.A)(+e, +r);
                    case '[object Error]':
                        return e.name == r.name && e.message == r.message;
                    case '[object RegExp]':
                    case '[object String]':
                        return e == r + '';
                    case '[object Map]':
                        var i = l.A;
                    case '[object Set]':
                        var s = 1 & n;
                        if ((i || (i = A.A), e.size != r.size && !s)) break;
                        var h = o.get(e);
                        if (h) return h == r;
                        ((n |= 2), o.set(e, r));
                        var p = a(i(e), i(r), n, u, f, o);
                        return (o.delete(e), p);
                    case '[object Symbol]':
                        if (v) return v.call(e) == v.call(r);
                }
                return !1;
            };
            var p = t(75363),
                g = Object.prototype.hasOwnProperty;
            let b = function (e, r, t, n, u, f) {
                var o = 1 & t,
                    a = (0, p.A)(e),
                    i = a.length;
                if (i != (0, p.A)(r).length && !o) return !1;
                for (var c = i; c--;) {
                    var d = a[c];
                    if (!(o ? d in r : g.call(r, d))) return !1;
                }
                var l = f.get(e),
                    A = f.get(r);
                if (l && A) return l == r && A == e;
                var s = !0;
                (f.set(e, r), f.set(r, e));
                for (var v = o; ++c < i;) {
                    var h = e[(d = a[c])],
                        b = r[d];
                    if (n) var x = o ? n(b, h, d, r, e, f) : n(h, b, d, e, r, f);
                    if (!(void 0 === x ? h === b || u(h, b, t, n, f) : x)) {
                        s = !1;
                        break;
                    }
                    v || (v = 'constructor' == d);
                }
                if (s && !v) {
                    var y = e.constructor,
                        m = r.constructor;
                    y != m &&
                        'constructor' in e &&
                        'constructor' in r &&
                        !('function' == typeof y && y instanceof y && 'function' == typeof m && m instanceof m) &&
                        (s = !1);
                }
                return (f.delete(e), f.delete(r), s);
            };
            var x = t(43906),
                y = t(47256),
                m = t(49593),
                _ = t(81942),
                j = '[object Arguments]',
                w = '[object Array]',
                E = '[object Object]',
                U = Object.prototype.hasOwnProperty;
            let I = function (e, r, t, u, f, o) {
                var i = (0, y.A)(e),
                    c = (0, y.A)(r),
                    d = i ? w : (0, x.A)(e),
                    l = c ? w : (0, x.A)(r);
                ((d = d == j ? E : d), (l = l == j ? E : l));
                var A = d == E,
                    s = l == E,
                    v = d == l;
                if (v && (0, m.A)(e)) {
                    if (!(0, m.A)(r)) return !1;
                    ((i = !0), (A = !1));
                }
                if (v && !A) return (o || (o = new n.A()), i || (0, _.A)(e) ? a(e, r, t, u, f, o) : h(e, r, d, t, u, f, o));
                if (!(1 & t)) {
                    var p = A && U.call(e, '__wrapped__'),
                        g = s && U.call(r, '__wrapped__');
                    if (p || g) {
                        var I = p ? e.value() : e,
                            S = g ? r.value() : r;
                        return (o || (o = new n.A()), f(I, S, t, u, o));
                    }
                }
                return !!v && (o || (o = new n.A()), b(e, r, t, u, f, o));
            };
            var S = t(83107);
            let L = function e(r, t, n, u, f) {
                return r === t || (null != r && null != t && ((0, S.A)(r) || (0, S.A)(t)) ? I(r, t, n, u, e, f) : r != r && t != t);
            };
        },
        15132: (e, r, t) => {
            t.d(r, { A: () => I });
            var n = /[^\x00-\x2f\x3a-\x40\x5b-\x60\x7b-\x7f]+/g,
                u = /[a-z][A-Z]|[A-Z]{2}[a-z]|[0-9][a-zA-Z]|[a-zA-Z][0-9]|[^a-zA-Z0-9 ]/,
                f = t(85599),
                o = '\ud800-\udfff',
                a = '\\u2700-\\u27bf',
                i = 'a-z\\xdf-\\xf6\\xf8-\\xff',
                c = 'A-Z\\xc0-\\xd6\\xd8-\\xde',
                d =
                    '\\xac\\xb1\\xd7\\xf7\\x00-\\x2f\\x3a-\\x40\\x5b-\\x60\\x7b-\\xbf\\u2000-\\u206f \\t\\x0b\\f\\xa0\\ufeff\\n\\r\\u2028\\u2029\\u1680\\u180e\\u2000\\u2001\\u2002\\u2003\\u2004\\u2005\\u2006\\u2007\\u2008\\u2009\\u200a\\u202f\\u205f\\u3000',
                l = "['’]",
                A = '[' + d + ']',
                s = '[' + i + ']',
                v = '[^' + o + d + '\\d+' + a + i + c + ']',
                h = '(?:\ud83c[\udde6-\uddff]){2}',
                p = '[\ud800-\udbff][\udc00-\udfff]',
                g = '[' + c + ']',
                b = '(?:' + s + '|' + v + ')',
                x = '(?:' + g + '|' + v + ')',
                y = '(?:' + l + '(?:d|ll|m|re|s|t|ve))?',
                m = '(?:' + l + '(?:D|LL|M|RE|S|T|VE))?',
                _ = '(?:[\\u0300-\\u036f\\ufe20-\\ufe2f\\u20d0-\\u20ff]|\ud83c[\udffb-\udfff])?',
                j = '[\\ufe0e\\ufe0f]?',
                w = '(?:\\u200d(?:' + ['[^' + o + ']', h, p].join('|') + ')' + j + _ + ')*',
                E = '(?:' + ['[' + a + ']', h, p].join('|') + ')' + (j + _ + w),
                U = RegExp(
                    [
                        g + '?' + s + '+' + y + '(?=' + [A, g, '$'].join('|') + ')',
                        x + '+' + m + '(?=' + [A, g + b, '$'].join('|') + ')',
                        g + '?' + b + '+' + y,
                        g + '+' + m,
                        '\\d*(?:1ST|2ND|3RD|(?![123])\\dTH)(?=\\b|[a-z_])|\\d*(?:1st|2nd|3rd|(?![123])\\dth)(?=\\b|[A-Z_])|\\d+',
                        E,
                    ].join('|'),
                    'g',
                );
            let I = function (e, r, t) {
                if (((e = (0, f.A)(e)), void 0 === (r = t ? void 0 : r))) {
                    var o;
                    return ((o = e), u.test(o)) ? e.match(U) || [] : e.match(n) || [];
                }
                return e.match(r) || [];
            };
        },
        16323: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = t(449);
            let u = (0, t(53234).A)(function (e, r, t) {
                return ((r = r.toLowerCase()), e + (t ? (0, n.A)(r) : r));
            });
        },
        18861: (e, r, t) => {
            t.d(r, { A: () => f });
            var n = t(33661);
            function u(e) {
                var r = -1,
                    t = null == e ? 0 : e.length;
                for (this.__data__ = new n.A(); ++r < t;) this.add(e[r]);
            }
            ((u.prototype.add = u.prototype.push =
                function (e) {
                    return (this.__data__.set(e, '__lodash_hash_undefined__'), this);
                }),
                (u.prototype.has = function (e) {
                    return this.__data__.has(e);
                }));
            let f = u;
        },
        19931: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = RegExp('[\\u200d\ud800-\udfff\\u0300-\\u036f\\ufe20-\\ufe2f\\u20d0-\\u20ff\\ufe0e\\ufe0f]');
            let u = function (e) {
                return n.test(e);
            };
        },
        21225: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r, t) {
                var n = -1,
                    u = e.length;
                (r < 0 && (r = -r > u ? 0 : u + r), (t = t > u ? u : t) < 0 && (t += u), (u = r > t ? 0 : (t - r) >>> 0), (r >>>= 0));
                for (var f = Array(u); ++n < u;) f[n] = e[n + r];
                return f;
            };
        },
        21916: (e, r, t) => {
            var n = t(23948);
            (t.o(n, 'ServerInsertedHTMLContext') &&
                t.d(r, {
                    ServerInsertedHTMLContext: function () {
                        return n.ServerInsertedHTMLContext;
                    },
                }),
                t.o(n, 'notFound') &&
                    t.d(r, {
                        notFound: function () {
                            return n.notFound;
                        },
                    }),
                t.o(n, 'redirect') &&
                    t.d(r, {
                        redirect: function () {
                            return n.redirect;
                        },
                    }),
                t.o(n, 'usePathname') &&
                    t.d(r, {
                        usePathname: function () {
                            return n.usePathname;
                        },
                    }),
                t.o(n, 'useRouter') &&
                    t.d(r, {
                        useRouter: function () {
                            return n.useRouter;
                        },
                    }),
                t.o(n, 'useSearchParams') &&
                    t.d(r, {
                        useSearchParams: function () {
                            return n.useSearchParams;
                        },
                    }),
                t.o(n, 'useServerInsertedHTML') &&
                    t.d(r, {
                        useServerInsertedHTML: function () {
                            return n.useServerInsertedHTML;
                        },
                    }));
        },
        23281: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = (0, t(42962).A)('toUpperCase');
        },
        23622: (e, r, t) => {
            t.d(r, { A: () => i });
            var n = t(81021),
                u = t(60672),
                f = t(44895),
                o = t(6028),
                a = t(69387);
            let i = (0, f.A)(function (e) {
                var r = (0, a.A)(e),
                    t = (0, n.A)(e, o.A);
                return ((r = 'function' == typeof r ? r : void 0) && t.pop(), t.length && t[0] === e[0] ? (0, u.A)(t, void 0, r) : []);
            });
        },
        27576: (e, r, t) => {
            let n;
            t.d(r, { A: () => a });
            let u = { randomUUID: 'undefined' != typeof crypto && crypto.randomUUID && crypto.randomUUID.bind(crypto) },
                f = new Uint8Array(16),
                o = [];
            for (let e = 0; e < 256; ++e) o.push((e + 256).toString(16).slice(1));
            let a = function (e, r, t) {
                if (u.randomUUID && !r && !e) return u.randomUUID();
                let a =
                    (e = e || {}).random ||
                    (
                        e.rng ||
                        function () {
                            if (!n && !(n = 'undefined' != typeof crypto && crypto.getRandomValues && crypto.getRandomValues.bind(crypto)))
                                throw Error('crypto.getRandomValues() not supported. See https://github.com/uuidjs/uuid#getrandomvalues-not-supported');
                            return n(f);
                        }
                    )();
                if (((a[6] = (15 & a[6]) | 64), (a[8] = (63 & a[8]) | 128), r)) {
                    t = t || 0;
                    for (let e = 0; e < 16; ++e) r[t + e] = a[e];
                    return r;
                }
                return (function (e, r = 0) {
                    return (
                        o[e[r + 0]] +
                        o[e[r + 1]] +
                        o[e[r + 2]] +
                        o[e[r + 3]] +
                        '-' +
                        o[e[r + 4]] +
                        o[e[r + 5]] +
                        '-' +
                        o[e[r + 6]] +
                        o[e[r + 7]] +
                        '-' +
                        o[e[r + 8]] +
                        o[e[r + 9]] +
                        '-' +
                        o[e[r + 10]] +
                        o[e[r + 11]] +
                        o[e[r + 12]] +
                        o[e[r + 13]] +
                        o[e[r + 14]] +
                        o[e[r + 15]]
                    );
                })(a);
            };
        },
        28566: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                var r = -1,
                    t = Array(e.size);
                return (
                    e.forEach(function (e) {
                        t[++r] = e;
                    }),
                    t
                );
            };
        },
        32528: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r, t) {
                for (var n = -1, u = null == e ? 0 : e.length; ++n < u;) if (t(r, e[n])) return !0;
                return !1;
            };
        },
        42962: (e, r, t) => {
            t.d(r, { A: () => a });
            var n = t(48307),
                u = t(19931),
                f = t(95568),
                o = t(85599);
            let a = function (e) {
                return function (r) {
                    r = (0, o.A)(r);
                    var t = (0, u.A)(r) ? (0, f.A)(r) : void 0,
                        a = t ? t[0] : r.charAt(0),
                        i = t ? (0, n.A)(t, 1).join('') : r.slice(1);
                    return a[e]() + i;
                };
            };
        },
        44895: (e, r, t) => {
            t.d(r, { A: () => o });
            var n = t(13209),
                u = t(88396),
                f = t(54717);
            let o = function (e, r) {
                return (0, f.A)((0, u.A)(e, r, n.A), e + '');
            };
        },
        45124: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                var r = -1,
                    t = Array(e.size);
                return (
                    e.forEach(function (e, n) {
                        t[++r] = [n, e];
                    }),
                    t
                );
            };
        },
        48307: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = t(21225);
            let u = function (e, r, t) {
                var u = e.length;
                return ((t = void 0 === t ? u : t), !r && t >= u ? e : (0, n.A)(e, r, t));
            };
        },
        50785: (e, r, t) => {
            t.d(r, { A: () => o });
            var n = t(86858),
                u = t(12134);
            let f = function (e, r, t) {
                    for (var n = t - 1, u = e.length; ++n < u;) if (e[n] === r) return n;
                    return -1;
                },
                o = function (e, r, t) {
                    return r == r ? f(e, r, t) : (0, n.A)(e, u.A, t);
                };
        },
        53234: (e, r, t) => {
            t.d(r, { A: () => a });
            var n = t(97291),
                u = t(99118),
                f = t(15132),
                o = RegExp("['’]", 'g');
            let a = function (e) {
                return function (r) {
                    return (0, n.A)((0, f.A)((0, u.A)(r).replace(o, '')), e, '');
                };
            };
        },
        54717: (e, r, t) => {
            t.d(r, { A: () => a });
            var n = t(66783),
                u = t(86746),
                f = t(13209),
                o = u.A
                    ? function (e, r) {
                          return (0, u.A)(e, 'toString', { configurable: !0, enumerable: !1, value: (0, n.A)(r), writable: !0 });
                      }
                    : f.A;
            let a = (0, t(76890).A)(o);
        },
        60672: (e, r, t) => {
            t.d(r, { A: () => d });
            var n = t(18861),
                u = t(74492),
                f = t(32528),
                o = t(81021),
                a = t(16628),
                i = t(6762),
                c = Math.min;
            let d = function (e, r, t) {
                for (var d = t ? f.A : u.A, l = e[0].length, A = e.length, s = A, v = Array(A), h = 1 / 0, p = []; s--;) {
                    var g = e[s];
                    (s && r && (g = (0, o.A)(g, (0, a.A)(r))), (h = c(g.length, h)), (v[s] = !t && (r || (l >= 120 && g.length >= 120)) ? new n.A(s && g) : void 0));
                }
                g = e[0];
                var b = -1,
                    x = v[0];
                e: for (; ++b < l && p.length < h;) {
                    var y = g[b],
                        m = r ? r(y) : y;
                    if (((y = t || 0 !== y ? y : 0), !(x ? (0, i.A)(x, m) : d(p, m, t)))) {
                        for (s = A; --s;) {
                            var _ = v[s];
                            if (!(_ ? (0, i.A)(_, m) : d(e[s], m, t))) continue e;
                        }
                        (x && x.push(m), p.push(y));
                    }
                }
                return p;
            };
        },
        63993: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                return function (r) {
                    return null == e ? void 0 : e[r];
                };
            };
        },
        66783: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                return function () {
                    return e;
                };
            };
        },
        67893: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = t(14752);
            let u = function (e, r) {
                return (0, n.A)(e, r);
            };
        },
        69387: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e) {
                var r = null == e ? 0 : e.length;
                return r ? e[r - 1] : void 0;
            };
        },
        74492: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = t(50785);
            let u = function (e, r) {
                return !!(null == e ? 0 : e.length) && (0, n.A)(e, r, 0) > -1;
            };
        },
        75576: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r, t) {
                switch (t.length) {
                    case 0:
                        return e.call(r);
                    case 1:
                        return e.call(r, t[0]);
                    case 2:
                        return e.call(r, t[0], t[1]);
                    case 3:
                        return e.call(r, t[0], t[1], t[2]);
                }
                return e.apply(r, t);
            };
        },
        76890: (e, r, t) => {
            t.d(r, { A: () => u });
            var n = Date.now;
            let u = function (e) {
                var r = 0,
                    t = 0;
                return function () {
                    var u = n(),
                        f = 16 - (u - t);
                    if (((t = u), f > 0)) {
                        if (++r >= 800) return arguments[0];
                    } else r = 0;
                    return e.apply(void 0, arguments);
                };
            };
        },
        80725: (e, r, t) => {
            t.d(r, { A: () => f });
            var n = t(60058),
                u = t(83107);
            let f = function (e) {
                return !0 === e || !1 === e || ((0, u.A)(e) && '[object Boolean]' == (0, n.A)(e));
            };
        },
        86858: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r, t, n) {
                for (var u = e.length, f = t + (n ? 1 : -1); n ? f-- : ++f < u;) if (r(e[f], f, e)) return f;
                return -1;
            };
        },
        88396: (e, r, t) => {
            t.d(r, { A: () => f });
            var n = t(75576),
                u = Math.max;
            let f = function (e, r, t) {
                return (
                    (r = u(void 0 === r ? e.length - 1 : r, 0)),
                    function () {
                        for (var f = arguments, o = -1, a = u(f.length - r, 0), i = Array(a); ++o < a;) i[o] = f[r + o];
                        o = -1;
                        for (var c = Array(r + 1); ++o < r;) c[o] = f[o];
                        return ((c[r] = t(i)), (0, n.A)(e, this, c));
                    }
                );
            };
        },
        90780: (e, r, t) => {
            t.d(r, { A: () => f });
            var n = t(93223),
                u = t(83107);
            let f = function (e) {
                return (0, u.A)(e) && (0, n.A)(e);
            };
        },
        91961: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r) {
                for (var t = -1, n = null == e ? 0 : e.length; ++t < n;) if (r(e[t], t, e)) return !0;
                return !1;
            };
        },
        95568: (e, r, t) => {
            t.d(r, { A: () => v });
            var n = t(19931),
                u = '\ud800-\udfff',
                f = '[\\u0300-\\u036f\\ufe20-\\ufe2f\\u20d0-\\u20ff]',
                o = '\ud83c[\udffb-\udfff]',
                a = '[^' + u + ']',
                i = '(?:\ud83c[\udde6-\uddff]){2}',
                c = '[\ud800-\udbff][\udc00-\udfff]',
                d = '(?:' + f + '|' + o + ')?',
                l = '[\\ufe0e\\ufe0f]?',
                A = '(?:\\u200d(?:' + [a, i, c].join('|') + ')' + l + d + ')*',
                s = RegExp(o + '(?=' + o + ')|' + ('(?:' + [a + f + '?', f, i, c, '[' + u + ']'].join('|')) + ')' + (l + d + A), 'g');
            let v = function (e) {
                return (0, n.A)(e) ? e.match(s) || [] : e.split('');
            };
        },
        97291: (e, r, t) => {
            t.d(r, { A: () => n });
            let n = function (e, r, t, n) {
                var u = -1,
                    f = null == e ? 0 : e.length;
                for (n && f && (t = e[++u]); ++u < f;) t = r(t, e[u], u, e);
                return t;
            };
        },
        99118: (e, r, t) => {
            t.d(r, { A: () => a });
            var n = (0, t(63993).A)({
                    À: 'A',
                    Á: 'A',
                    Â: 'A',
                    Ã: 'A',
                    Ä: 'A',
                    Å: 'A',
                    à: 'a',
                    á: 'a',
                    â: 'a',
                    ã: 'a',
                    ä: 'a',
                    å: 'a',
                    Ç: 'C',
                    ç: 'c',
                    Ð: 'D',
                    ð: 'd',
                    È: 'E',
                    É: 'E',
                    Ê: 'E',
                    Ë: 'E',
                    è: 'e',
                    é: 'e',
                    ê: 'e',
                    ë: 'e',
                    Ì: 'I',
                    Í: 'I',
                    Î: 'I',
                    Ï: 'I',
                    ì: 'i',
                    í: 'i',
                    î: 'i',
                    ï: 'i',
                    Ñ: 'N',
                    ñ: 'n',
                    Ò: 'O',
                    Ó: 'O',
                    Ô: 'O',
                    Õ: 'O',
                    Ö: 'O',
                    Ø: 'O',
                    ò: 'o',
                    ó: 'o',
                    ô: 'o',
                    õ: 'o',
                    ö: 'o',
                    ø: 'o',
                    Ù: 'U',
                    Ú: 'U',
                    Û: 'U',
                    Ü: 'U',
                    ù: 'u',
                    ú: 'u',
                    û: 'u',
                    ü: 'u',
                    Ý: 'Y',
                    ý: 'y',
                    ÿ: 'y',
                    Æ: 'Ae',
                    æ: 'ae',
                    Þ: 'Th',
                    þ: 'th',
                    ß: 'ss',
                    Ā: 'A',
                    Ă: 'A',
                    Ą: 'A',
                    ā: 'a',
                    ă: 'a',
                    ą: 'a',
                    Ć: 'C',
                    Ĉ: 'C',
                    Ċ: 'C',
                    Č: 'C',
                    ć: 'c',
                    ĉ: 'c',
                    ċ: 'c',
                    č: 'c',
                    Ď: 'D',
                    Đ: 'D',
                    ď: 'd',
                    đ: 'd',
                    Ē: 'E',
                    Ĕ: 'E',
                    Ė: 'E',
                    Ę: 'E',
                    Ě: 'E',
                    ē: 'e',
                    ĕ: 'e',
                    ė: 'e',
                    ę: 'e',
                    ě: 'e',
                    Ĝ: 'G',
                    Ğ: 'G',
                    Ġ: 'G',
                    Ģ: 'G',
                    ĝ: 'g',
                    ğ: 'g',
                    ġ: 'g',
                    ģ: 'g',
                    Ĥ: 'H',
                    Ħ: 'H',
                    ĥ: 'h',
                    ħ: 'h',
                    Ĩ: 'I',
                    Ī: 'I',
                    Ĭ: 'I',
                    Į: 'I',
                    İ: 'I',
                    ĩ: 'i',
                    ī: 'i',
                    ĭ: 'i',
                    į: 'i',
                    ı: 'i',
                    Ĵ: 'J',
                    ĵ: 'j',
                    Ķ: 'K',
                    ķ: 'k',
                    ĸ: 'k',
                    Ĺ: 'L',
                    Ļ: 'L',
                    Ľ: 'L',
                    Ŀ: 'L',
                    Ł: 'L',
                    ĺ: 'l',
                    ļ: 'l',
                    ľ: 'l',
                    ŀ: 'l',
                    ł: 'l',
                    Ń: 'N',
                    Ņ: 'N',
                    Ň: 'N',
                    Ŋ: 'N',
                    ń: 'n',
                    ņ: 'n',
                    ň: 'n',
                    ŋ: 'n',
                    Ō: 'O',
                    Ŏ: 'O',
                    Ő: 'O',
                    ō: 'o',
                    ŏ: 'o',
                    ő: 'o',
                    Ŕ: 'R',
                    Ŗ: 'R',
                    Ř: 'R',
                    ŕ: 'r',
                    ŗ: 'r',
                    ř: 'r',
                    Ś: 'S',
                    Ŝ: 'S',
                    Ş: 'S',
                    Š: 'S',
                    ś: 's',
                    ŝ: 's',
                    ş: 's',
                    š: 's',
                    Ţ: 'T',
                    Ť: 'T',
                    Ŧ: 'T',
                    ţ: 't',
                    ť: 't',
                    ŧ: 't',
                    Ũ: 'U',
                    Ū: 'U',
                    Ŭ: 'U',
                    Ů: 'U',
                    Ű: 'U',
                    Ų: 'U',
                    ũ: 'u',
                    ū: 'u',
                    ŭ: 'u',
                    ů: 'u',
                    ű: 'u',
                    ų: 'u',
                    Ŵ: 'W',
                    ŵ: 'w',
                    Ŷ: 'Y',
                    ŷ: 'y',
                    Ÿ: 'Y',
                    Ź: 'Z',
                    Ż: 'Z',
                    Ž: 'Z',
                    ź: 'z',
                    ż: 'z',
                    ž: 'z',
                    Ĳ: 'IJ',
                    ĳ: 'ij',
                    Œ: 'Oe',
                    œ: 'oe',
                    ŉ: "'n",
                    ſ: 's',
                }),
                u = t(85599),
                f = /[\xc0-\xd6\xd8-\xf6\xf8-\xff\u0100-\u017f]/g,
                o = RegExp('[\\u0300-\\u036f\\ufe20-\\ufe2f\\u20d0-\\u20ff]', 'g');
            let a = function (e) {
                return (e = (0, u.A)(e)) && e.replace(f, n).replace(o, '');
            };
        },
    },
]);
